...
export function ResultPage({
  ...
  // Resolve the current player id for the rewarded ad claim
  useEffect(() => {
    let active = true;
    (async () => {
      try {
        ...
        /* ignore — rewarded ad simply won't show */
      }
    })();
    return () => { active = false; };
  }, []);
  ...
  const isBest = outcome.score >= stats.highestScore && outcome.score > 0;

  const handleReward = () => {
    // Refresh stats so the UI reflects the bonus XP
    onRewardEarned();
  };
    ...
    <div className="mx-auto w-full max-w-2xl px-4 pb-24 pt-6 md:pb-10">
      ...
      <div className="card relative overflow-hidden p-6 text-center animate-pop-in sm:p-8">
        ...
        {isBest && (
          <span className="chip mx-auto mb-3 bg-warning-500/15 text-warning-600 dark:text-warning-300">
            <Star size={13} /> New personal best!
          </span>
        )}
        <div className="mx-auto mb-3 grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br from-primary-500 to-primary-700 text-white shadow-glow">
          <span className="font-display text-3xl font-extrabold tabular-nums">{outcome.score}</span>
        </div>
        <h1 className="font-display text-2xl font-extrabold">Challenge complete</h1>
        <p className="mx-auto mt-2 max-w-sm text-sm text-ink-500 dark:text-ink-400">{message}</p>
        {saving && (
          <p className="mx-auto mt-3 inline-flex items-center gap-1.5 text-xs font-medium text-primary-500">
            <Loader2 size={13} className="animate-spin" /> Saving your score…
          </p>
        )}
        {saveError && !saving && (
          <p className="mx-auto mt-3 inline-flex items-center gap-1.5 text-xs font-medium text-error-500">
            <AlertCircle size={13} /> Couldn't save to the server. Your local stats are still updated.
          </p>
        )}
      </div>
      ...
      {wasGuest && !saving && (
        <div className="card mt-4 flex flex-col items-center gap-3 p-5 text-center animate-fade-in">
          ...
          <button onClick={onCreateAccount} className="btn-primary w-full sm:w-auto">
      ...
      {adsActive && outcome.gameResultId && (
        <div className="mt-4">
          <RewardedAd
            open={rewardedOpen}
            playerId={playerId}
      ...
      <section className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
        <StatCard icon={<CheckCircle size={20} />} label="Correct" value={outcome.correctAnswers} accent="success" />
        <StatCard icon={<ListChecks size={20} />} label="Attempted" value={outcome.attemptedQuestions} accent="primary" />
        <StatCard icon={<Zap size={20} />} label="XP Earned" value={`+${outcome.xpEarned}`} accent="accent" />
        <StatCard icon={<Flame size={20} />} label="Streak" value={`${stats.streak}d`} accent="warning" />
        <StatCard icon={<Star size={20} />} label="Highest" value={stats.highestScore} accent="primary" />
      ...
      <section className="card mt-4 p-5">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-ink-500 dark:text-ink-400">Level {lp.level} · {lp.levelName}</p>
            <p className="font-display text-lg font-bold">Total XP: {stats.xp}</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-medium text-ink-500 dark:text-ink-400">Next</p>
            <p className="font-display text-lg font-bold text-primary-500">{lp.next ?? 'MAX'}</p>
          </div>
        </div>
        <ProgressBar value={lp.next ? stats.xp : 1} max={lp.next ?? 1} color="primary" />
      </section>
      ...
      <section className="mt-5 grid gap-3 sm:grid-cols-3">
        ...
        <button onClick={onHome} className="btn-ghost">
          <Home size={18} /> Daily Challenge
        </button>
        <button onClick={onLeaderboard} className="btn-outline">
